...
class LLOHandler:
    ...
    def _emit_llo_attributes(
        ...
        for message in all_messages:
            ...
            if role in [ROLE_SYSTEM, ROLE_USER]:
                ...
                # If it's from a completion/output attribute, treat as output
        ...
        # Create event body with the new format
        event_body = {}
        if input_messages:
        ...
        # Get scope name from span
        scope_name = span.instrumentation_scope.name
        
        # Get event logger with the span's scope
    ...
    def _collect_gen_ai_prompt_messages(self, span: ReadableSpan, attributes: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for index in sorted(prompt_content_matches.keys(), key=int):
            ...
            messages.append({"content": value, "role": role, "source": "prompt"})
    ...
    def _collect_gen_ai_completion_messages(
        ...
        for index in sorted(completion_content_matches.keys(), key=int):
            ...
            messages.append({"content": value, "role": role, "source": "completion"})
    ...
    def _collect_traceloop_messages(self, span: ReadableSpan, attributes: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        traceloop_attrs = [
            (TRACELOOP_ENTITY_INPUT, ROLE_USER, "input"),  # Treat input as user role
            (TRACELOOP_ENTITY_OUTPUT, ROLE_ASSISTANT, "output"),  # Treat output as assistant role
        ]

        for attr_key, role, source in traceloop_attrs:
            if attr_key in attributes:
                messages.append({"content": attributes[attr_key], "role": role, "source": source})
        ...
        for attr_key, role, source in crewai_attrs:
            if attr_key in attributes:
                messages.append({"content": attributes[attr_key], "role": role, "source": source})
    ...
    def _collect_openlit_messages(self, span: ReadableSpan, attributes: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        openlit_attrs = [
            (OPENLIT_PROMPT, ROLE_USER, "prompt"),  # Assume user role for direct prompts
            (OPENLIT_COMPLETION, ROLE_ASSISTANT, "completion"),  # Assume assistant role for completions
            (OPENLIT_REVISED_PROMPT, ROLE_SYSTEM, "prompt"),  # Assume system role for revised prompts
            (OPENLIT_AGENT_ACTUAL_OUTPUT, ROLE_ASSISTANT, "output"),  # Assume assistant role for agent output
            (OPENLIT_AGENT_HUMAN_INPUT, ROLE_USER, "input"),  # Assume user role for agent human input
        ]

        for attr_key, role, source in openlit_attrs:
            if attr_key in attributes:
                messages.append({"content": attributes[attr_key], "role": role, "source": source})
    ...
    def _collect_openinference_messages(self, span: ReadableSpan, attributes: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for attr_key, role, source in openinference_direct_attrs:
            if attr_key in attributes:
                messages.append({"content": attributes[attr_key], "role": role, "source": source})
        ...
        for index in sorted(input_messages.keys(), key=int):
            ...
            messages.append({"content": value, "role": role, "source": "input"})
        ...
        for index in sorted(output_messages.keys(), key=int):
            ...
            messages.append({"content": value, "role": role, "source": "output"})
